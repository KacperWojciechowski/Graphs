...
void generate_throughtput(std::string input_file_path,
...
public:
  /* Constructors */
  AdjMatrix(std::string file_path, std::string throughtput_file_path,
  ...
  int32_t throughtput_belman_ford(unsigned int searched_throughtput,
                                  unsigned int vertex, bool log);
...
private:
  ...
  /* Private functions */
  unsigned int find_index(std::vector<coord> &edges, unsigned int x,
                          unsigned int y);
