...
void Graph::Benchmark::run(Graphs::Graph &graph, std::string identifier,
                           std::string file_path, uint16_t iterations,
                           Mode mode, bool bench_log, bool alg_log) {
  std::fstream file;
  switch (mode) {
  case Mode::append:
  ...
  if (file.good()) {
    this->color_benchmark(graph, identifier, iterations, file, bench_log,
                          alg_log);
  } else {
...
void Graph::Benchmark::color_benchmark(Graphs::Graph &graph,
                                       ...
                                       uint16_t iterations, std::fstream &file,
                                       bool bench_log, bool alg_log) {
