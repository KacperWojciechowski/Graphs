...
namespace Graphs {
AdjMatrix::AdjMatrix(std::string file_path, std::string throughtput_file_path,
                     std::string name, std::string type)
    : matrix(nullptr), nodes_amount(0), graph_name(name), graph_type(type),
      distance(nullptr), prev_node(nullptr), throughtput(nullptr) {
...
/*
        ...
        None
*/
AdjMatrix::AdjMatrix(const Graph &graph) : throughtput(nullptr) {
...
void AdjMatrix::saveGraphML(std::string file_path) {
  ...
  // nodes data
  for (uint32_t i = 0; i < this->nodes_amount;
       i++) // Tworzy wiersze nod�w <node id="n0"/>
  {
    file << "	<node id=\"n" << i;
    file << "\"/>\n";
  }
...
float AdjMatrix::estrada_index() {
  ...
  for (uint32_t i = 0; i < this->nodes_amount; i++) {
    for (uint32_t j = 0; j < this->nodes_amount; j++) {
      ...
      {
        float root = std::sqrt(static_cast<float>(degrees[i] + degrees[j] - 2) /
                               (pow(degrees[i], 2)));
        index += root;
      }
    }
...
void AdjMatrix::setEdge(const EdgeInfo &edge) {
  if (edge.source < this->nodes_amount &&
      edge.destination < this->nodes_amount) {
...
uint32_t AdjMatrix::nodesAmount() const { return this->nodes_amount; }

EdgeInfo AdjMatrix::findEdge(const EdgeInfo &edge) const {
  if (edge.source > this->nodes_amount or
      edge.destination > this->nodes_amount) {
  ...
  if (matrix[edge.source - 1][edge.destination - 1] == 0) {
    ...
    return {edge.source, edge.destination,
            matrix[edge.source - 1][edge.destination - 1]};
  }
}
  ...
  for (uint32_t i = 0; i < this->nodes_amount; i++) {
    ...
    for (uint32_t x = 0; x < this->nodes_amount; x++) {
      ...
      if (log) {
        ...
        for (uint32_t j = 0; j < this->nodes_amount; j++) {
          for (auto itr = this->prev_node[j].begin();
               itr != this->prev_node[j].end(); itr++)
            std::cout << *itr << ", ";
          std::cout << std::endl;
        }
        std::cout << std::endl;
...
int32_t AdjMatrix::throughtput_belman_ford(uint32_t searched_throughtput,
                                           uint32_t vertex, bool log) {
  // set default values for the distances and previous vertices
  for (uint32_t i = 0; i < this->nodes_amount; i++) {
    this->distance[i] = 0x7FFFFFFF;
    this->prev_node[i].clear();
    ...
    for (uint32_t x = 0; x < this->nodes_amount; x++) {
      for (uint32_t j = 0; j < this->nodes_amount; j++) {
        ...
        // requirement, and is not already saved, save it in prev_node vector.
                 ...
                 this->throughtput[x][j] >= searched_throughtput &&
                 std::find(this->prev_node[j].begin(), this->prev_node[j].end(),
                           x) == this->prev_node[j].end()) {
          // remember the change was found to continue search
          change_found = true;
          this->prev_node[j].push_back(x);
        }
...
uint32_t AdjMatrix::find_index(std::vector<coord> &edges, uint32_t x,
                               uint32_t y) {
  uint32_t size = static_cast<uint32_t>(edges.size());
...
void Data::generate_weighted_graph(std::string file_path,
...
void Data::generate_throughtput(std::string input_file_path,
